...
mod tests {
    ...
    #[test]
    fn test_token_parse_block_start_end() {
        let mut evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize("{ 1 2 }").unwrap();
        assert_eq!(tokens.len(), 4);
        assert!(matches!(tokens[0], EbaseToken::BlockStart));
        assert!(matches!(tokens[1], EbaseToken::Number(_)));
        assert!(matches!(tokens[2], EbaseToken::Number(_)));
        assert!(matches!(tokens[3], EbaseToken::BlockEnd));
    }

    #[test]
    fn test_token_parse_operators() {
        let mut evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize("+ * /").unwrap();
        assert_eq!(tokens.len(), 3);
    }

    #[test]
    fn test_token_parse_comparisons() {
        let mut evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize("= == != < > <= >=").unwrap();
        assert!(tokens.len() >= 5);
    }
    ...
    #[test]
    fn test_token_parse_symbol() {
        let mut evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize("@left @right").unwrap();
        assert_eq!(tokens.len(), 2);
        if let EbaseToken::Symbol(s) = &tokens[0] {
            assert_eq!(s, "left");
        }
        if let EbaseToken::Symbol(s) = &tokens[1] {
            assert_eq!(s, "right");
        }
    }

    #[test]
    fn test_token_parse_negative_number() {
        let mut evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize("-5.5").unwrap();
        assert_eq!(tokens.len(), 1);
        if let EbaseToken::Number(n) = &tokens[0] {
            assert!((*n - (-5.5)).abs() < 0.001);
        }
    }

    #[test]
    fn test_token_parse_subtraction_operator() {
        let mut evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize("5 - 3").unwrap();
        assert_eq!(tokens.len(), 3);
        // The middle token should be the - operator
        if let EbaseToken::Ident(s) = &tokens[1] {
            assert_eq!(s, "-");
        }
    }

    #[test]
    fn test_token_parse_scientific_notation() {
        let mut evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize("1e5 2.5E3").unwrap();
        assert_eq!(tokens.len(), 2);
    }

    #[test]
    fn test_execute_ifelse_condition() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();

        // true case - use comparison to get boolean
        evaluator.stack.clear();
        evaluator.evaluate("5 3 > { 10 } { 20 } ifelse", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(10.0)));

        // false case - use comparison to get boolean
        evaluator.stack.clear();
        evaluator.evaluate("3 5 > { 10 } { 20 } ifelse", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(20.0)));
    }
    ...
    fn test_execute_if_condition() {
        ...
        // true case - executes block using comparison
        evaluator.stack.clear();
        evaluator.evaluate("5 3 > { 42 } if", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(42.0)));

        // false case - doesn't execute block
        evaluator.stack.clear();
        evaluator.evaluate("3 5 > { 42 } if", &props).unwrap();
        assert!(evaluator.stack.is_empty());
    }

    #[test]
    fn test_execute_comparison_ops() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();

        // Less than
        evaluator.stack.clear();
        evaluator.evaluate("3 5 <", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));

        // Greater than
        evaluator.stack.clear();
        evaluator.evaluate("5 3 >", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));
    ...
    #[test]
    fn test_execute_egms() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();

        let result = evaluator.evaluate("\"object_name\" egms", &props).unwrap();
        if let EbaseResult::Egms { name } = result {
            assert_eq!(name, "object_name");
        } else {
            panic!("Expected Egms result");
        }
    }

    #[test]
    fn test_execute_clsref() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();

        let result = evaluator.evaluate("100 200 10 \"::ofml::go::GoYLTrans\" clsref", &props).unwrap();
        if let EbaseResult::ClsRef { class, params } = result {
            assert_eq!(class, "::ofml::go::GoYLTrans");
            assert_eq!(params.len(), 3);
        } else {
            panic!("Expected ClsRef result");
        }
    }

    #[test]
    fn test_execute_and_or_not() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();

        // and - using comparisons to produce booleans
        evaluator.stack.clear();
        evaluator.evaluate("1 0 > 2 1 > and", &props).unwrap(); // true and true = true
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));

        evaluator.stack.clear();
        evaluator.evaluate("1 0 > 0 1 > and", &props).unwrap(); // true and false = false
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(false)));

        // or - using comparisons to produce booleans
        evaluator.stack.clear();
        evaluator.evaluate("0 1 > 2 1 > or", &props).unwrap(); // false or true = true
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));

        evaluator.stack.clear();
        evaluator.evaluate("0 1 > 0 2 > or", &props).unwrap(); // false or false = false
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(false)));

        // not - using comparison to produce boolean
        evaluator.stack.clear();
        evaluator.evaluate("1 0 > not", &props).unwrap(); // not true = false
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(false)));

        evaluator.stack.clear();
        evaluator.evaluate("0 1 > not", &props).unwrap(); // not false = true
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));
    }
    ...
    fn test_execute_neg() {
        ...
        evaluator.stack.clear();
        evaluator.evaluate("5 neg", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(-5.0)));
    }
    ...
    fn test_execute_dup() {
        ...
        evaluator.stack.clear();
        evaluator.evaluate("42 dup", &props).unwrap();
        assert_eq!(evaluator.stack.len(), 2);
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(42.0)));
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(42.0)));
    }
    ...
    fn test_execute_exch() {
        ...
        assert_eq!(a, Some(EbaseValue::Float(1.0)));
        assert_eq!(b, Some(EbaseValue::Float(2.0)));
    }

    #[test]
    fn test_execute_mul_div() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();

        // Multiplication
        evaluator.stack.clear();
        evaluator.evaluate("3 4 *", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(12.0)));

        // Division
        evaluator.stack.clear();
        evaluator.evaluate("10 2 /", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(5.0)));
    }

    #[test]
    fn test_execute_add_sub_aliases() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();

        // add alias
        evaluator.stack.clear();
        evaluator.evaluate("3 4 add", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(7.0)));

        // sub alias
        evaluator.stack.clear();
        evaluator.evaluate("10 3 sub", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(7.0)));

        // mul alias
        evaluator.stack.clear();
        evaluator.evaluate("5 6 mul", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(30.0)));

        // div alias
        evaluator.stack.clear();
        evaluator.evaluate("20 4 div", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Float(5.0)));
    }

    #[test]
    fn test_execute_eq_ne_aliases() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();

        // eq alias
        evaluator.stack.clear();
        evaluator.evaluate("5 5 eq", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));

        // ne alias
        evaluator.stack.clear();
        evaluator.evaluate("5 3 ne", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));

        // lt alias
        evaluator.stack.clear();
        evaluator.evaluate("3 5 lt", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));

        // gt alias
        evaluator.stack.clear();
        evaluator.evaluate("5 3 gt", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));

        // le alias
        evaluator.stack.clear();
        evaluator.evaluate("5 5 le", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));

        // ge alias
        evaluator.stack.clear();
        evaluator.evaluate("5 5 ge", &props).unwrap();
        assert_eq!(evaluator.stack.pop(), Some(EbaseValue::Bool(true)));
    }
    ...
    fn test_unterminated_string() {
        let mut evaluator = EbaseEvaluator::new();
        let result = evaluator.tokenize("\"unclosed string");
        assert!(result.is_err());
    }
    ...
    fn test_unbalanced_braces() {
        ...
        let result = evaluator.evaluate("{ 1 2", &props);
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_ebase_token_clone() {
        let token = EbaseToken::String("hello".to_string());
        let cloned = token.clone();
        assert_eq!(token, cloned);
    }

    #[test]
    fn test_ebase_value_clone() {
        let value = EbaseValue::Float(3.14);
        let cloned = value.clone();
        assert_eq!(value, cloned);
    }

    #[test]
    fn test_ebase_result_clone() {
        let result = EbaseResult::Egms {
            name: "test".to_string(),
        };
        let cloned = result.clone();
        if let (EbaseResult::Egms { name: n1 }, EbaseResult::Egms { name: n2 }) = (&result, &cloned) {
            assert_eq!(n1, n2);
        }
    }
    ...
    fn test_string_escape_sequences() {
        let mut evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize(r#""hello\nworld""#).unwrap();
        if let EbaseToken::String(s) = &tokens[0] {
            assert!(s.contains('\n'));
        }
    }
    ...
    fn test_unexpected_character() {
        let mut evaluator = EbaseEvaluator::new();
        let result = evaluator.tokenize("§");
        assert!(result.is_err());
    }

    #[test]
    fn test_invalid_number() {
        let evaluator = EbaseEvaluator::new();
        let result = evaluator.tokenize(".e.");
        // This should either fail or parse as something
    ...
    fn test_unterminated_variable() {
        let evaluator = EbaseEvaluator::new();
        let props = HashMap::new();
        // Unterminated ${VAR - no closing brace
        let result = evaluator.substitute_variables("${VAR", &props);
        assert!(result.is_err());
    }

    #[test]
    fn test_unterminated_variable_with_default() {
        let evaluator = EbaseEvaluator::new();
        let props = HashMap::new();
        // Unterminated ${VAR:-default - no closing brace
        let result = evaluator.substitute_variables("${VAR:-default", &props);
        assert!(result.is_err());
    }

    #[test]
    fn test_variable_not_found_no_default() {
        let evaluator = EbaseEvaluator::new();
        let props = HashMap::new();
        // Variable not found and no default provided
        let result = evaluator.substitute_variables("${UNKNOWN}", &props);
        assert!(result.is_err());
    }
    ...
    fn test_string_escape_tab() {
        let evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize(r#""hello\tworld""#).unwrap();
        if let EbaseToken::String(s) = &tokens[0] {
            assert!(s.contains('\t'));
        } else {
    ...
    fn test_string_escape_backslash() {
        let evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize(r#""path\\to\\file""#).unwrap();
        if let EbaseToken::String(s) = &tokens[0] {
            assert!(s.contains('\\'));
        } else {
            panic!("Expected string token");
    ...
    fn test_string_escape_quote() {
        let evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize(r#""say \"hello\"""#).unwrap();
        if let EbaseToken::String(s) = &tokens[0] {
            assert!(s.contains('"'));
        } else {
            panic!("Expected string token");
    ...
    fn test_symbol_token() {
        let evaluator = EbaseEvaluator::new();
        let tokens = evaluator.tokenize("@symbol_name").unwrap();
        if let EbaseToken::Symbol(s) = &tokens[0] {
    ...
    #[test]
    fn test_unbalanced_opening_brace() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();
        // Unbalanced opening brace
        let result = evaluator.evaluate(r#"{ 1 2"#, &props);
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_evaluate_to_number_empty_stack() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();
        // Simple numeric expression that leaves stack empty
        let result = evaluator.evaluate_to_number("", &props);
        assert!(result.is_err());
    }

    #[test]
    fn test_evaluate_to_number_with_parse_fallback() {
        let mut evaluator = EbaseEvaluator::new();
        let props = HashMap::new();
        // Simple number that can be parsed directly
        let result = evaluator.evaluate_to_number("42.5", &props);
        assert!(result.is_ok());
    ...
    fn test_negative_number_vs_operator() {
        let evaluator = EbaseEvaluator::new();
        // Negative sign followed by space should be operator
        let tokens = evaluator.tokenize("10 - 5").unwrap();
        assert_eq!(tokens.len(), 3);
        if let EbaseToken::Ident(op) = &tokens[1] {
            assert_eq!(op, "-");
        }
    }
