            ...
            match key.as_str() {
                ...
                "prj" | "projection" => {
                    if let Some(ref mut tex) = material.texture {
                        tex.projection = TextureProjection::from_str(value);
                    }
                }
                _ => {
                    // Ignore unknown keys
    ...
    pub fn to_material_def(&self) -> MaterialDef {
        ...
        if let Some(ref tex) = self.texture {
            ...
            if let Some(ref prj) = self.projection {
                tex_def.projection = TextureProjection::from_str(prj);
            }
...
mod tests {
    ...
    fn test_texture_projection() {
        assert_eq!(
            TextureProjection::from_str("prjx"),
            TextureProjection::ProjectX
        );
        assert_eq!(
            TextureProjection::from_str("prjy"),
            TextureProjection::ProjectY
        );
        assert_eq!(
            TextureProjection::from_str("prjz"),
            TextureProjection::ProjectZ
        );
        assert_eq!(
            TextureProjection::from_str("cyl"),
            TextureProjection::Cylindrical
        );
        assert_eq!(
            TextureProjection::from_str("sph"),
