...
#[cfg(feature = "tui")]
use crate::tui::app::{App, TableInfo};
#[cfg(feature = "tui")]
...
fn is_standard_table(name: &str) -> bool {
    STANDARD_TABLES.iter().any(|&s| s.eq_ignore_ascii_case(name))
}
...
fn render_table_item(table: &TableInfo) -> ListItem<'static> {
    ...
    let cols_preview = if table.columns.len() > 3 {
        format!("{}, {} +{}",
            table.columns.get(0).map(|s| s.as_str()).unwrap_or(""),
...
pub fn render_table_view(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let header_text = vec![
        Line::from(vec![
            ...
            Span::styled(&table_info.name, Style::default().add_modifier(Modifier::BOLD)),
            Span::styled(
                format!(" ({} Zeilen, {} Spalten)", table_info.row_count, table_info.columns.len()),
                Style::default().fg(Color::DarkGray),
            ),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            Span::styled("←→: Scroll | ↑↓: Zeile | Esc: Zurück", Style::default().fg(Color::DarkGray)),
        ]),
    ];
    ...
    // Build header row
    let header_cells: Vec<Cell> = table_info.columns
        .iter()
        .skip(app.table_scroll_x)
        .take(10) // Show max 10 columns
        .map(|c| Cell::from(c.clone()).style(Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)))
        .collect();

    let header_row = Row::new(header_cells).height(1);

    // Build data rows
    let rows: Vec<Row> = app.table_rows
        .iter()
        .map(|row| {
            let cells: Vec<Cell> = row.values
                .iter()
                .skip(app.table_scroll_x)
                .take(10)
                .map(|v| {
    ...
    let table = Table::new(rows, widths)
        .header(header_row)
        .block(Block::default().borders(Borders::ALL).title(format!(" Daten{} ", scroll_info)))
        .row_highlight_style(
            Style::default()
                .bg(Color::Blue)
                .fg(Color::White),
