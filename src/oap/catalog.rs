...
impl CatalogNode {
    ...
    pub fn article_with_variant(id: &str, name: &str, depth: u8, series_ref: Option<String>, variant_code: Option<String>) -> Self {
        Self {
            id: id.to_string(),
            name: name.to_string(),
            node_type: NodeType::Article,
    ...
    pub fn folder_count(&self) -> usize {
        let self_count = if self.node_type == NodeType::Folder { 1 } else { 0 };
        self_count + self.children.iter().map(|c| c.folder_count()).sum::<usize>()
    }

    /// Count articles only
    pub fn article_count(&self) -> usize {
        let self_count = if self.node_type == NodeType::Article { 1 } else { 0 };
        self_count + self.children.iter().map(|c| c.article_count()).sum::<usize>()
    }
...
impl XcfCatalog {
    ...
    pub fn get_text_variant_with_fallback(&self, id: &str, variant: &str, language: &str) -> Option<&str> {
    ...
    pub fn get_variant(&self, article_id: &str, variant_code: &str) -> Option<&VariantDefinition> {
        self.variants.get(&(article_id.to_string(), variant_code.to_string()))
    }
...
impl CatalogLoader {
    ...
    fn parse_structure_csv(content: &str) -> Result<Vec<StructureEntry>, CatalogError> {
        ...
        for line in content.lines() {
            ...
            entries.push(StructureEntry { id, variant_code, depth, node_type });
        }
    ...
    fn parse_text_csv(content: &str) -> Result<HashMap<(String, String, String), String>, CatalogError> {
        ...
        for line in content.lines() {
            ...
            let variant_str = fields[1].trim_matches('"');
            let variant = if variant_str == "default" { String::new() } else { variant_str.to_string() };
    ...
    fn parse_variant_csv(content: &str) -> Result<HashMap<(String, String), VariantDefinition>, CatalogError> {
    ...
    fn build_tree(
        ...
        for entry in entries {
            ...
            let name = texts
                .get(&(entry.id.clone(), variant_str.to_string(), language.to_string()))
                .or_else(|| texts.get(&(entry.id.clone(), variant_str.to_string(), "de".to_string())))
                .or_else(|| texts.get(&(entry.id.clone(), variant_str.to_string(), "en".to_string())))
            ...
            while stack.len() > 1 && stack.last().map(|(d, _)| *d >= entry.depth).unwrap_or(false) {
                stack.pop();
            }
...
impl std::fmt::Display for CatalogError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CatalogError::IoError(path, msg) => write!(f, "IO error reading {}: {}", path.display(), msg),
...
pub fn find_manufacturer_catalogs(mfr_path: &Path) -> Vec<CatalogInfo> {
    ...
    if master_cat.exists() {
        ...
        for region_entry in std::fs::read_dir(&master_cat).into_iter().flatten() {
            if let Ok(region) = region_entry {
                let region_path = region.path();
                if region_path.is_dir() {
                    for version_entry in std::fs::read_dir(&region_path).into_iter().flatten() {
    ...
    if let Ok(entries) = std::fs::read_dir(mfr_path) {
        for entry in entries.flatten() {
            ...
            if series_path.is_dir() {
                let series_name = series_path.file_name()
                    .and_then(|n| n.to_str())
                    .unwrap_or("")
                    .to_string();
...
pub fn load_all_manufacturer_catalogs(mfr_path: &Path, language: &str) -> Vec<(String, XcfCatalog)> {
...
impl ManufacturerRegistry {
                    ...
                    "general" => {
                        match key {
                            ...
                            _ if key.starts_with("series_name.") => {
                                let series_id = key.strip_prefix("series_name.").unwrap();
                                registry.series_names.insert(
    ...
    pub fn get_series_name(&self, series_id: &str) -> Option<&str> {
        self.series_names.get(&series_id.to_lowercase()).map(|s| s.as_str())
    }
}
...
pub fn build_aggregated_catalog(
    ...
    for cat_info in &catalog_infos {
        ...
        let display_name = registry.as_ref()
                ...
                let effective_children = if series_catalog.root.children.len() == 1 {
                    ...
                    if first_child.node_type == NodeType::Folder
                        && (first_child.name.to_lowercase() == display_name.to_lowercase()
                            || first_child.name.to_lowercase().contains(&cat_info.name.to_lowercase()))
                    {
...
"#;
        ...
        assert_eq!(texts.get(&("@FOLDER1".to_string(), String::new(), "de".to_string())), Some(&"Kategorie Eins".to_string()));
        assert_eq!(texts.get(&("@FOLDER1".to_string(), String::new(), "en".to_string())), Some(&"Category One".to_string()));
    }

    #[test]
    fn test_build_tree() {
        let entries = vec![
            StructureEntry { id: "@FOLDER1".to_string(), variant_code: None, depth: 1, node_type: NodeType::Folder },
            StructureEntry { id: "@FOLDER2".to_string(), variant_code: None, depth: 2, node_type: NodeType::Folder },
            StructureEntry { id: "ART1".to_string(), variant_code: None, depth: 3, node_type: NodeType::Article },
            StructureEntry { id: "@FOLDER3".to_string(), variant_code: None, depth: 1, node_type: NodeType::Folder },
        ];
        let mut texts = HashMap::new();
        texts.insert(("@FOLDER1".to_string(), String::new(), "de".to_string()), "Hauptkategorie".to_string());
        texts.insert(("@FOLDER2".to_string(), String::new(), "de".to_string()), "Unterkategorie".to_string());
    ...
    fn test_load_bisley_catalog() {
        ...
        for child in &catalog.root.children {
            println!("  {} - {} ({} children)",
                child.id, child.name, child.children.len());
        }
    ...
    fn test_find_manufacturer_catalogs() {
        ...
        for cat in &catalogs {
            println!("  {} (master: {}) -> {}", cat.name, cat.is_master, cat.path.display());
        }
    ...
    fn test_find_kn_master_catalogs() {
        ...
        assert!(master_catalogs.len() >= 3, "K+N should have at least 3 master catalogs");
    ...
    fn test_load_sedus_registry() {
        ...
        let reg = reg.unwrap();
        println!("\nSedus Registry:");
        println!("  Manufacturer: {} ({})", reg.manufacturer_name, reg.manufacturer_id);
    ...
    fn test_build_sedus_aggregated_catalog() {
        ...
        assert!(catalog.is_some(), "Should build aggregated catalog for Sedus");
        ...
        assert!(catalog.root.children.len() > 20, "Sedus should have many series");

        // Check that series names are resolved
        let has_se_air = catalog.root.children.iter().any(|c| c.name == "se:air");
        let has_quarterback = catalog.root.children.iter().any(|c| c.name == "quarterback");
        assert!(has_se_air, "Should have se:air series with display name");
        assert!(has_quarterback, "Should have quarterback series with display name");
