...
mod snapshot_tests {
    ...
    use ofml_interpreter::oap::families::{FamilyConfiguration, FamilyLoader, FamilyProperty, ProductFamily};
    use ofml_interpreter::oap::{Manufacturer, PriceResult};
    use ofml_interpreter::tui::app::{App, Screen};
    use ofml_interpreter::tui::ui::render;
    ...
    fn test_family_config_with_price() {
        ...
        app.family_properties = vec![
            FamilyProperty {
                ...
                prop_type: ofml_interpreter::oap::families::PropertyType::Choice,
                required: true,
                options: vec![
                    ofml_interpreter::oap::families::PropertyOption {
                        value: "2G3".to_string(),
    ...
    fn test_family_config_with_surcharges() {
        ...
        use ofml_interpreter::oap::Surcharge;
        app.family_price = Some(PriceResult::new(
            rust_decimal::Decimal::from(599),
            vec![
                Surcharge {
    ...
    fn test_real_data_price_display() {
        ...
        if let Some(family) = ai_family {
            ...
            assert!(buffer_str.contains("599") || buffer_str.contains("Preis"),
    ...
    fn test_price_update_on_property_change() {
        ...
        if let Some(family) = ai_family {
            ...
            let opt_other = prop.options.iter().find(|o| !o.value.contains("166") && !o.value.contains("167") && !o.value.contains("168"));
            ...
            let price_without = engine.calculate_family_price("sex", &family, &config_without, chrono::Local::now().date_naive());
            let price_with = engine.calculate_family_price("sex", &family, &config_with, chrono::Local::now().date_naive());

            if let (Some(p1), Some(p2)) = (&price_without, &price_with) {
                // Verify prices are different
                assert_ne!(p1.total_price, p2.total_price,
                    "Price should change when selecting option with embedded surcharge code");

                // Verify the one with 166 has a surcharge
                assert!(!p2.surcharges.is_empty(),
